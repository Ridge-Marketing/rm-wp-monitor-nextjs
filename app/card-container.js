  ...
  return (
    <>
      ...
      {groupedSites?.map((Category) => (
        <section
          key={Category.website_group}
          className="mb-10 md:mb-16 lg:mb-20 bg-white px-8 pt-1"
        >
          <h2 className={`font-semibold uppercase text-4xl mt-6 pb-8`}>
            {Category.website_group.replace(/[^a-zA-Z0-9 ]/g, " ")}
          </h2>
          {migrations.map((migration) => {
            if (
              Category.website_group.replace(/[^a-zA-Z0-9 ]/g, " ") ==
              migration.website_group.replace(/[^a-zA-Z0-9 ]/g, " ")
            ) {
          ...
          <div className="flex justify-between">
            <BasicCard
              siteType={"PRODUCTION"}
              pages={Category.pages.prod}
              migration={prod}
            />
            <BasicCard
              siteType={"STAGING"}
              pages={Category.pages.stage}
              migration={stage}
            />
            <BasicCard
              siteType={"DEVELOPMENT"}
              pages={Category.pages.dev}
              migration={dev}
            />
            <BasicCard
              siteType={"LOCAL"}
              pages={Category.pages.local}
              migration={""}
            />
          </div>
        </section>
